import { Dimension } from "../components/DimensionType";
import { DrawImageProps } from "../components/IDrawImageProps";
...
export function getDrawImagePropsFromPage(
  ...
  const offsetX = getImageOffset(
    canvas.width,
    centerX,
    imageToCanvasScale
  );
  const offsetY = getImageOffset(
    canvas.height,
    centerY,
    imageToCanvasScale
  );
 ...
 */
export function getCanvasDimension(dimension: Dimension): number {

  const pixelRatio = window.devicePixelRatio;
  if (dimension.valueOf() === "width") {
    return window.innerWidth * pixelRatio;
  } else {
    return window.innerHeight * pixelRatio;
  }
}
